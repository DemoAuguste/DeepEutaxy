...
def get_mobilenet_v2(num_classes=10):
    model = MobileNetv2((32,32,3), num_classes)
    optimizer = keras.optimizers.SGD(learning_rate=0.1)


    # optimizer = keras.optimizers.Adagrad(lr=0.1, epsilon=1e-6, decay=0.0005)
    # optimizer = keras.optimizers.Adadelta(lr=0.1, epsilon=1e-6)
    model.compile(loss=keras.metrics.categorical_crossentropy, optimizer=optimizer, metrics=['accuracy'])
    return model

def get_mobilenet_v2_uncompiled(num_classes=10):
    model = MobileNetv2((32,32,3), num_classes)
